...
class PlaybackService : MediaSessionService() {
                ...
                override fun onCustomCommand(
                    ...
                    if (customCommand.customAction == ACTION_TOGGLE_STAR) {
                        ...
                        currentTrack?.id?.let { trackId ->
                            ...
                            _events.tryEmit(
                                PlaybackEvent.StarToggled(trackId = trackId, isStarred = previousStarred),
                            )
                        }
    ...
    fun setRepeatMode(mode: String, emitQueueState: Boolean = false) {
        ...
        _events.tryEmit(PlaybackEvent.RepeatModeChanged(mode = mode))
    }
    ...
    private fun checkScrobble() {
        ...
        if (accumulatedListenMs.toFloat() / durationMs >= threshold) {
            ...
            // Notify UI so it can update play counts
            _events.tryEmit(PlaybackEvent.Scrobbled(trackId = songId))
        }
    }
    ...
    private fun emitStateChange() {
        _events.tryEmit(PlaybackEvent.StateChanged(state = getState()))
    }
    ...
    private fun emitProgressEvent() {
        ...
        _events.tryEmit(
            PlaybackEvent.Progress(
                positionMs = rawPosition + seekTimeOffsetMs,
                durationMs = duration,
                bufferedMs = buffered + seekTimeOffsetMs,
            ),
        )
    }

    private fun emitTrackChange() {
        _events.tryEmit(PlaybackEvent.TrackChanged(track = currentTrack, queueIndex = queueIndex))
    }
    ...
    private fun emitError(
        ...
        _events.tryEmit(
            PlaybackEvent.PlaybackError(
                message = message,
                trackId = trackId,
                category = classification?.category?.jsValue,
    ...
    private fun emitQueueStateChanged() {
        _events.tryEmit(
            PlaybackEvent.QueueStateChanged(
    ...
    private fun emitClippingEvent(peakOverDb: Float) {
        _events.tryEmit(
            PlaybackEvent.Clipping(
                peakOverDb = peakOverDb.toDouble(),
