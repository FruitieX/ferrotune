...
object DownloadManagerHolder {
    ...
    private val _events = MutableSharedFlow<DownloadStateEventPayload>(extraBufferCapacity = 8)
    val events: SharedFlow<DownloadStateEventPayload> = _events.asSharedFlow()
    ...
    private fun broadcast(
        ...
        Log.d(
            TAG,
            "Emitting download event; downloads=${info.joinToString { "${it.contentId}:${it.status}:${it.bytesDownloaded}/${it.bytesTotal}" }} paused=$paused notMet=$notMet",
        )
        mainHandler.post { _events.tryEmit(payload) }
    }
