...
| `android/feature/player/QueueSheet.kt` + `NowPlayingScreen.kt` + `MiniPlayerBar.kt` | Queue sheet (cover rows, now-playing highlight, per-row play/move/remove menu, shuffle/repeat/clear), full-screen player (seeded backdrop, swipe down to dismiss, swipe sideways to skip, star + rating actions, queue button), and mini player (tap or swipe up to expand, swipe sideways to skip) | `QueueSheet()`, `NowPlayingScreen()`, `MiniPlayerBar()` |
| `android/core/media/AudioState.kt` + `FerrotuneApiClient.kt` | `TrackInfo` carries the current track's `starred`/`userRating` parsed from queue-window `QueueSong` JSON, so player surfaces can show and mutate flags | `TrackInfo`, `QueueSong`, `parseSong()`, `songToTrackInfo()` |
...
| `android/core/actions/SongFlagsStore.kt` | Optimistic star/rating overlay over API mutations with rollback on failure; partial overrides keep unloaded ratings intact during bulk starring | `SongFlags`, `SongFlagsOverride`, `SongFlagsStore`, `setStarred()`, `setStarredBulk()`, `setRating()`, `clear()` |
...
| `android/core/designsystem/components/` | Shared Compose building blocks | `CoverArt` (seeded gradient fallback), `MediaCard`/`ShelfCard` (shelf + grid cards with play overlay), `MediaRow` (circular-cover and multi-select support), `SectionHeader`, `DetailHeader` (gradient detail header with badges/actions), `StarButton`/`RatingStars`/`RatingStarsDisplay`, `ShimmerBox`/`MediaRowSkeletonList`/`MediaCardSkeleton`, `SortMenu`, `PagingListFooter`, `ErrorState`, `EmptyState`, `LoadingState` |
