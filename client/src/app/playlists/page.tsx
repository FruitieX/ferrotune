...
import { Plus, ListMusic, Upload, Clock, Folder, FolderOpen, FolderPlus, ChevronRight, Home, GripVertical } from "lucide-react";
...
function PlaylistsPageContent() {
  ...
  const movePlaylistMutation = useMutation({
    mutationFn: async ({ playlist, targetFolderPath }: { playlist: Playlist; targetFolderPath: string }) => {
      const client = getClient();
      if (!client) throw new Error("Not connected");
  ...
  const displayFolders = useMemo(() => {
    ...
    return folderItems.folders.filter(f => f.name.toLowerCase().includes(query));
            ...
            viewMode === "grid" ? (
              <VirtualizedGrid
                items={[
                  ...
                  ...displayPlaylists.map(p => ({ type: "playlist" as const, data: p })),
                ]}
                renderItem={(item) => 
                  item.type === "folder" ? (
                ...
                getItemKey={(item) => item.type === "folder" ? `folder-${item.data.path}` : item.data.id}
              />
            ) : (
              <VirtualizedList
                items={[
      ...
      <DragOverlay>
        {activeDragPlaylist && (
          <div className="bg-background/90 backdrop-blur-sm rounded-lg border border-border shadow-lg p-3 flex items-center gap-3">
            ...
            <span className="font-medium">{getPlaylistDisplayName(activeDragPlaylist)}</span>
          </div>
        )}
      </DragOverlay>
    </DndContext>
...
function DroppableBreadcrumb({ path, onClick, children }: DroppableBreadcrumbProps) {
