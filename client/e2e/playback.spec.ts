...
test.describe("Playback", () => {
  ...
  test("player bar is visible when authenticated", async ({
  ...
  test("player bar has playback controls", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/");
    await page.waitForTimeout(1000);
...
test.describe("Repeat Mode Behavior", () => {
  test("repeat one only loops track on natural end, not on next click", async ({
    authenticatedPage: page,
  }, testInfo) => {
...
test.describe("Queue End Behavior", () => {
  test("clicking next on last track shows 'Not playing' state", async ({
    authenticatedPage: page,
  }) => {
    await playFirstSong(page);
    await waitForPlayerReady(page);
  ...
  test("queue panel hides 'Now Playing' section when queue ends", async ({
    authenticatedPage: page,
  }) => {
    await playFirstSong(page);
    await waitForPlayerReady(page);
    ...
    const queuePanel = (await sidebar.isVisible().catch(() => false))
      ? sidebar
      : sheet;
  ...
  test("clicking play after queue ends restarts from first track", async ({
    authenticatedPage: page,
  }) => {
    await playFirstSong(page);
    await waitForPlayerReady(page);
  ...
  test("track list does not show any track as active when queue ends", async ({
    authenticatedPage: page,
  }) => {
    // Navigate to Test Album
    await page.goto("/library");
    await page.waitForSelector('[data-testid="album-card"], article', {
      timeout: 10000,
    });

    const testAlbum = page
      .locator('[data-testid="album-card"], article')
      .filter({ hasText: "Test Album" });
    await testAlbum.click();
    await page.waitForURL(/\/library\/albums\//, { timeout: 10000 });
    await page.waitForSelector('[data-testid="song-row"], [role="row"]', {
      timeout: 10000,
    });

    // Double-click first track to play
    const firstTrack = page
      .locator('[data-testid="song-row"], [role="row"]')
      .first();
    ...
    const activeTrackIndicator = page.locator(
      '[data-testid="song-row"] .text-primary, [role="row"] .text-primary',
    );
    await expect(activeTrackIndicator).toHaveCount(0);
  });

  test("can play new tracks after queue ends", async ({
    authenticatedPage: page,
  }) => {
    await playFirstSong(page);
    await waitForPlayerReady(page);
