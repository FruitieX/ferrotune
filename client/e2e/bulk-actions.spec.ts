...
test.describe("Bulk Actions", () => {
  test.describe("Selection", () => {
    ...
    test("clicking checkbox selects a song", async ({
    ...
    test("bulk actions bar shows correct count", async ({
      authenticatedPage: page,
    }) => {
      // Select first song
    ...
    test("can select all songs", async ({ authenticatedPage: page }) => {
      ...
      const selectAllButton = bulkBar.getByRole("button", {
        name: /select all/i,
      });
    ...
    test("can clear selection", async ({ authenticatedPage: page }) => {
      ...
      // Click clear button
      const clearButton = bulkBar.getByRole("button", {
  ...
  test.describe("Bulk Play Actions", () => {
    ...
    test("can shuffle play selected songs", async ({
      authenticatedPage: page,
    }) => {
      // Select all songs
      const firstSong = page.locator('[data-testid="song-row"]').first();
      ...
      const shuffleButton = bulkBar.getByRole("button", {
        name: /shuffle play/i,
      });
    ...
    test("can add selected songs to play next", async ({
      authenticatedPage: page,
    }) => {
      ...
      const playNextButton = bulkBar.getByRole("button", {
        name: /play next/i,
      });
      ...
      // Verify toast
      await expect(
        page.locator("[data-sonner-toast]").filter({ hasText: /added|next/i }),
      ).toBeVisible({ timeout: 3000 });
    });

    test("can add selected songs to queue", async ({
      authenticatedPage: page,
    }) => {
      // First start playback
      await page.locator('[data-testid="song-row"]').first().dblclick();
      ...
      const addToQueueButton = bulkBar.getByRole("button", {
        name: /add to queue/i,
      });
  ...
  test.describe("Bulk Favorites", () => {
    ...
    test("can add selected songs to favorites", async ({
      authenticatedPage: page,
    }) => {
      // Select songs
      const firstSong = page.locator('[data-testid="song-row"]').first();
      ...
      const addFavoritesButton = bulkBar.getByRole("button", {
      ...
      await expect(
        page
          .locator("[data-sonner-toast]")
          .filter({ hasText: /favorite|starred/i }),
      ).toBeVisible({ timeout: 3000 });
    });

    test("can remove selected songs from favorites", async ({
      authenticatedPage: page,
    }) => {
      ...
      const removeFavoritesButton = bulkBar.getByRole("button", {
        name: /remove from favorites/i,
      });
      if (
        await removeFavoritesButton
        ...
        await expect(
          page
            .locator("[data-sonner-toast]")
            .filter({ hasText: /removed|unfavorite/i }),
        ).toBeVisible({ timeout: 3000 });
      }
    });
  });
  ...
  test.describe("Bulk Add to Playlist", () => {
    test("can add selected songs to existing playlist", async ({
      authenticatedPage: page,
    }) => {
      ...
      const addToPlaylistButton = bulkBar.getByRole("button", {
      ...
      if (
        ...
        // Verify toast
        await expect(
          page.locator("[data-sonner-toast]").filter({ hasText: /added to/i }),
        ).toBeVisible({ timeout: 3000 });
      }
    });
  });
  ...
  test.describe("Multi-selection Keyboard Shortcuts", () => {
    ...
    test("ctrl+click selects multiple songs", async ({
      authenticatedPage: page,
    }) => {
      const firstSong = page.locator('[data-testid="song-row"]').first();
    ...
    test("shift+click selects range of songs", async ({
      authenticatedPage: page,
    }) => {
      const firstSong = page.locator('[data-testid="song-row"]').first();
      const thirdSong = page.locator('[data-testid="song-row"]').nth(2);
