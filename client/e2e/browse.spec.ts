...
test.describe("Browse Library", () => {
  test.describe("Library Page", () => {
    test("displays library page with tabs", async ({
      ...
      await expect(
        libraryNav.getByRole("link", { name: "Songs" }),
      ).toBeVisible();
      await expect(
    ...
    test("albums tab is active by default", async ({
      authenticatedPage: page,
    }) => {
  ...
  test.describe("Favorites", () => {
    ...
    test("can switch between favorite tabs", async ({
      authenticatedPage: page,
    }) => {
      await page.goto("/favorites");
      ...
      await expect(page.getByRole("tab", { name: /artists/i })).toHaveAttribute(
        "data-state",
        "active",
      );
      ...
      await expect(page.getByRole("tab", { name: /songs/i })).toHaveAttribute(
        "data-state",
        "active",
      );
    });
  ...
  test.describe("Navigation", () => {
    test("sidebar navigation to library works", async ({
      authenticatedPage: page,
    }, testInfo) => {
    ...
    test("sidebar navigation to favorites works", async ({
      authenticatedPage: page,
    }, testInfo) => {
  ...
  test.describe("Genre Details", () => {
    test("can navigate to genre detail page", async ({
      authenticatedPage: page,
    }) => {
      await page.goto("/library/genres");

      // Wait for genres to load and click the first one
      const genreCard = page
        .locator('a[href^="/library/genres/details"]')
        .first();
    ...
    test("genre detail page shows header and albums", async ({
      authenticatedPage: page,
    }) => {
      await page.goto("/library/genres");
      ...
      await expect(
        actionButtons.filter({ hasText: "Play" }).first(),
      ).toBeVisible();
      await expect(
        actionButtons.filter({ hasText: "Shuffle" }).first(),
      ).toBeVisible();
    });

    test("genre detail page has view mode toggle", async ({
      authenticatedPage: page,
    }) => {
      await page.goto("/library/genres");
      ...
      await expect(
        stickyHeader
          .locator("button")
          .filter({ has: page.locator('[class*="lucide-grid"]') }),
