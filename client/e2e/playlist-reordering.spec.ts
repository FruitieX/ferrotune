...
test.describe("Playlist Reordering", () => {
  ...
  async function createPlaylistWithSongs(
    ...
    for (let i = 0; i < Math.min(count, 3); i++) {
      ...
      if (
        await addToPlaylistItem.isVisible({ timeout: 2000 }).catch(() => false)
      ) {
        ...
        if (
          await playlistOption.isVisible({ timeout: 3000 }).catch(() => false)
        ) {
  ...
  test.describe("Drag Handle Visibility", () => {
    test("drag handle appears on hover when sort is Custom", async ({
    ...
    test("drag handle is hidden when filter is active", async ({
      authenticatedPage: page,
    }) => {
    ...
    test("drag handle is hidden when sort is not Custom", async ({
      authenticatedPage: page,
    }) => {
    ...
    test("drag handle is hidden in grid view", async ({
      authenticatedPage: page,
    }) => {
  ...
  test.describe("Drag and Drop Reordering", () => {
    test("can drag song to new position", async ({
      authenticatedPage: page,
    }) => {
      ...
      if (initialCount >= 2) {
        ...
        const _secondSongText = await songRows.nth(1).textContent();
    ...
    test("reorder persists after page reload", async ({
      authenticatedPage: page,
    }) => {
      ...
      if (count >= 2) {
        ...
        if (await dragHandle.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          if (handleBox && secondRowBox) {
            ...
            const afterReloadFirstText = await page
  ...
  test.describe("Drag During Selection", () => {
    test("drag is disabled when songs are selected", async ({
      authenticatedPage: page,
    }) => {
      ...
      if (await checkbox.isVisible({ timeout: 2000 }).catch(() => false)) {
        ...
        const _dragHandle = page.locator("svg.lucide-grip-vertical");
