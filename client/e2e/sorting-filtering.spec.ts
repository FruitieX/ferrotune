...
test.describe("Sorting and Filtering", () => {
  test.describe("Library Songs View", () => {
    test.beforeEach(async ({ authenticatedPage: page }) => {
      ...
      // Switch to list view to see song rows
    ...
    test("filter shows no results message when empty", async ({
    ...
    test("clearing filter shows all songs", async ({
      authenticatedPage: page,
    }) => {
      const filterInput = page.getByRole("textbox", { name: /filter/i });
      await filterInput.fill("First");
    ...
    test("can change sort order", async ({ authenticatedPage: page }) => {
      ...
      // Verify songs are sorted (either ascending or descending by name)
      const firstSongText = await page
        .locator('[data-testid="song-row"]')
        .first()
        .textContent();
      // "First" or "Third" would be at the top depending on sort direction
    ...
    test("can toggle sort direction", async ({ authenticatedPage: page }) => {
      ...
      const ascDescToggle = page.getByRole("menuitem", {
  ...
  test.describe("View Mode Toggle", () => {
    ...
    test("can switch back to list view", async ({
      authenticatedPage: page,
    }) => {
  ...
  test.describe("Column Visibility", () => {
    ...
    test("column visibility toggle is visible", async ({
      authenticatedPage: page,
    }) => {
    ...
    test("can open column visibility menu", async ({
      authenticatedPage: page,
    }) => {
      const columnsButton = page.getByRole("button", {
        name: /toggle columns/i,
    ...
    test("can toggle column visibility", async ({
      authenticatedPage: page,
    }) => {
      const columnsButton = page.getByRole("button", {
        name: /toggle columns/i,
      ...
      const artistOption = page.getByRole("menuitemcheckbox", {
        name: /artist/i,
      });
  ...
  test.describe("Playlist Detail Sorting", () => {
    test("playlist sort options include Custom", async ({
      authenticatedPage: page,
    }) => {
  ...
  test.describe("Filter Persistence", () => {
    test("filter persists during navigation within same page", async ({
      authenticatedPage: page,
    }) => {
      await page.goto("/library/songs");
      await waitForPageReady(page);
