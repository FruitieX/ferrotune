...
test.describe("Search", () => {
  ...
  test("search with query parameter populates input", async ({
  ...
  test("search returns results for artists", async ({
    authenticatedPage: page,
  }) => {
    ...
    const artistCards = page
      .locator('[data-testid="media-card"], [class*="group"]')
      .filter({
  ...
  test("empty search shows appropriate state", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/search");
    await page.waitForTimeout(500);
  ...
  test("no results for gibberish search", async ({
    authenticatedPage: page,
  }) => {
    await page.goto("/search");
    await page.waitForTimeout(500);
    ...
    const _hasNoResultsMessage = await noResults.isVisible().catch(() => false);

    // The search should complete without errors (either empty or no results message)
    expect(page.url()).toContain("/search");
  });

  test("sidebar navigation to search works", async ({
    authenticatedPage: page,
  }, testInfo) => {
    await page.goto("/");
    await page.waitForTimeout(500);
