...
pub async fn scan_library(
    ...
    for folder in folders {
        ...
        scan_folder(pool, config, &folder.path, full).await?;
    }
...
async fn scan_folder(pool: &SqlitePool, _config: &Config, folder_path: &str, full: bool) -> Result<()> {
        ...
        .filter_map(|e| e.ok())
    {
        ...
        match extract_metadata(path).await {
            Ok(metadata) => {
                match upsert_song(pool, metadata, relative_path).await {
                    Ok(is_new) => {
                        if is_new {
                            added += 1;
                        } else {
        ...
        if scanned % 100 == 0 {
            tracing::info!("Progress: {} files scanned, {} added, {} updated, {} errors", 
                scanned, added, updated, errors);
        }
    }
...
async fn extract_metadata(path: &Path) -> Result<SongMetadata> {
    ...
    // Extract tags
    let (title, artist, album, album_artist, track_number, disc_number, year, genre) = if let Some(tag) = tag {
        let title = tag.title()
            .map(|s| s.to_string())
            .unwrap_or_else(|| {
                path.file_stem()
        ...
        let album_artist = tag.get_string(&lofty::tag::ItemKey::AlbumArtist)
            .map(|s| s.to_string());
        ...
        (title, "Unknown Artist".to_string(), None, None, None, None, None, None)
    };
...
async fn upsert_song(pool: &SqlitePool, metadata: SongMetadata, file_path: String) -> Result<bool> {
    ...
    let artist_name = metadata.album_artist.as_ref()
...
async fn get_or_create_artist(tx: &mut sqlx::Transaction<'_, sqlx::Sqlite>, name: &str) -> Result<String> {
    ...
    if let Some((id,)) = existing {
        ...
        sqlx::query(
            "INSERT INTO artists (id, name, album_count) VALUES (?, ?, 0)"
        )
        .bind(&artist_id)
        .bind(name)
        .execute(&mut **tx)
...
async fn get_or_create_album(
    ...
    let existing: Option<(String,)> = sqlx::query_as(
        "SELECT id FROM albums WHERE name = ? COLLATE NOCASE AND artist_id = ?"
    )
    .bind(name)
    .bind(artist_id)
    .fetch_optional(&mut **tx)
