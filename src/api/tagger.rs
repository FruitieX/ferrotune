...
pub async fn discover_orphaned_files(
    ...
    let session_track_ids: HashSet<String> =
        match crate::api::tagger_session::get_session_track_ids(&state.database, user.user_id).await
        {
...
pub async fn delete_staged_file(
    ...
    if let Ok(session_id) =
        crate::api::tagger_session::get_or_create_session(&state.database, user.user_id).await
    {
...
/// GET /api/tagger/staged/:id/stream
///
...
/// GET /api/tagger/staged/:id/cover
///
...
/// POST /api/tagger/check-conflicts
///
